...
const ResEmoji = {wood:'🪵', planks:'🪚', stone:'🧱', grain:'🌾', flour:'⚙️', bread:'🍞', fish:'🐟', meat:'\ud83c\udf56', water:'🚰', pigs:'🐖', coal:'⚫', iron_ore:'⛓️', gold_ore:'✨', iron:'🔩', gold:'🪙', tools:'🧰', weapons:'⚔️', beer:'🍺'};
...
const BP = {
  ...
  mason:  {name:'Stonecutter', icon:'🧱', cost:{wood:10}, needs:'mountain', every:4000, out:{stone:1}},
...
function newMap(){
  ...
  for(let y=0;y<WORLD;y++){
    ...
    for(let x=0;x<WORLD;x++){
      const r=rand(); let type='grass';
      if(r<0.12) type='water';
      else if(r<0.34) type='forest';
      else if(r<0.52) type='mountain';
      row.push({type, stock: defaultStock(type), building:null, deposit:null, dStock:0, surveyed:false});
    }
    map.push(row);
  }
  for(let y=0;y<WORLD;y++) for(let x=0;x<WORLD;x++){
    if(map[y][x].type==='water' && Math.random()<0.10){
      map[y][x].type='fish'; map[y][x].stock=defaultStock('fish');
    }
  }
  return map;
}
...
function claimCircle(owner,cx,cy,r){
  const tgt = owner==='player'? State.control.player : State.control.ai;
  const r2=r*r;
  for(let y=Math.max(0,cy-r); y<Math.min(WORLD,cy+r+1); y++){
    for(let x=Math.max(0,cx-r); x<Math.min(WORLD,cx+r+1); x++){
      const dx=x-cx, dy=y-cy; if(dx*dx+dy*dy<=r2) tgt[y][x]=true;
    }
  }
}
...
function updateControl(){
  resetControl();
  for(let y=0;y<WORLD;y++) for(let x=0;x<WORLD;x++){
    const t=State.map[y][x]; const b=t.building; if(!b) continue;
    const bp=BP[b.kind]; if((bp.claim||0)<=0 || b.paused) continue;
    if(b.kind==='hq'){
      if(b.owner==='player') claimCircle('player',x,y,bp.claim);
      else claimCircle('ai',x,y,bp.claim);
    } else {
      if(sumGarrison(b)>=1){
        if(b.owner==='player') claimCircle('player',x,y,bp.claim);
        else claimCircle('ai',x,y,bp.claim);
...
function friendlyCoverageAt(owner,x,y){
  for(let yy=0; yy<WORLD; yy++) for(let xx=0; xx<WORLD; xx++){
    const t=State.map[yy][xx]; const b=t.building; if(!b) continue;
    const bp=BP[b.kind]; if(!bp.claim||b.paused||b.owner!==owner) continue;
    if(b.kind==='watch' || b.kind==='castle'){
      if(sumGarrison(b)>=1){
        const dx=xx-x, dy=yy-y; if(dx*dx+dy*dy <= bp.claim*bp.claim) return b;
      }
    }
  }
  return null;
...
function renderResBar(){
  const pills = [
    ...ResList.map(k=>`<div class="pill" title="${k}"><span>${ResEmoji[k]||'•'}</span><span class="val">${Math.floor(State.res[k]||0)}</span></div>`),
    `<div class="pill" title="Soldiers"><span>🙂</span><span class="val">${State.soldiers}</span></div>`,
    `<div class="pill" title="Veterans"><span>😬</span><span class="val">${State.veterans}</span></div>`
  ].join('');
  elResBar.innerHTML = pills;
}
function placeHintFor(k){
  if(k==='acre') return 'Field: free, only on GRASS and next to a Farm, within your territory.';
  if(k==='farm') return 'Farm: produces only with adjacent Fields.';
...
function renderPalette(){
  const order=['lumber','forester','sawmill','mason','farm','acre','well','mill','bakery','fisher','fish_farm','hunter','pig_farm','slaughter','brewery','coal_mine','iron_mine','gold_mine','furnace','gold_smelter','toolsmith','weaponsmith','barracks','training','watch','castle'];
  elBuildPalette.innerHTML = order.map(k=>{
    const bp=BP[k];
    const cost = Object.entries(bp.cost||{}).map(([r,v])=> v>0? `${ResEmoji[r]||r} ${v}`:null).filter(Boolean).join(' · ');
    return `<button data-kind="${k}" title="${bp.name}"><span class="title">${bp.icon} ${bp.name}</span><span class="cost">${cost||'free'}</span></button>`;
  }).join('');
  elBuildPalette.querySelectorAll('button').forEach(btn=>{
    btn.addEventListener('click',()=>{
      selectedKind = btn.dataset.kind;
      elBuildPalette.querySelectorAll('button').forEach(b=>b.classList.remove('active'));
      btn.classList.add('active');
      if(elPlaceHint) elPlaceHint.innerHTML = placeHintFor(selectedKind);
    });
  });
  const def = elBuildPalette.querySelector('button[data-kind="lumber"]'); if(def) def.classList.add('active');
  if(elPlaceHint) elPlaceHint.innerHTML = placeHintFor('lumber');
}
...
function drawMinimap(){
  const ctx=elMini.getContext('2d'); const W=elMini.width, H=elMini.height;
  ctx.clearRect(0,0,W,H); const sx=W/WORLD, sy=H/WORLD;
  for(let y=0;y<WORLD;y++) for(let x=0;x<WORLD;x++){
    const p = State.control.player[y][x]; const a = State.control.ai[y][x];
    let col = 'rgba(15,23,42,0.8)';
    if(p && a) col = 'rgba(127,29,29,0.9)'; else if(p) col = 'rgba(30,58,138,0.9)';
    ctx.fillStyle = col; ctx.fillRect(Math.floor(x*sx), Math.floor(y*sy), Math.ceil(sx), Math.ceil(sy));
  }
  ctx.strokeStyle='#fff'; ctx.lineWidth=1; ctx.strokeRect(State.cam.x*sx, State.cam.y*sy, VIEW_W*sx, VIEW_H*sy);
}

/* ===================== Context Menu ===================== */
let ctxEl=null;
function closeMenu(){ if(ctxEl){ ctxEl.remove(); ctxEl=null; } }
function openMenu(px,py,x,y){
  closeMenu(); if(!isVisibleToPlayer(x,y)) return;
  const t=State.map[y][x]; const b=t.building; const div=document.createElement('div'); div.className='ctxmenu'; let html='';
  if(b && b.owner==='player'){
    const bp=BP[b.kind]; const gsum = bp.cap? ((b.garrisonS||0)+(b.garrisonV||0)) : 0;
    html+=`<div class=title>${bp.icon} ${bp.name}${bp.cap?` · Garrison: ${gsum}/${bp.cap}`:''}</div>`;
    html+=`<div class=hint>Status: ${b.paused?'⏸️ paused':'▶️ active'}${(b.idle && !b.paused)?' · <span style="color:#f87171">✖ idle</span>':''}</div>`;
    const canDemo = (b.kind!=='hq');
    html+=`<div class=actions>`+
      `<button data-act=toggle>${b.paused?'Resume ▶️':'Pause ⏸️'}</button>`+
      (canDemo?`<button data-act=demolish>Demolish 🧹</button>`:'')+
      `<button data-act=close>Close</button></div>`;
  }
  else if(b && b.owner==='ai'){
    const bp=BP[b.kind];
    html+=`<div class=title>👾 [AI] ${bp.icon} ${bp.name}</div><div class=hint>In your vision.</div><div class=actions><button data-act=close>Close</button></div>`;
  }
    ...
    html+=`<div class=title>⛰ Mountain (${x+1},${y+1})</div><div class=hint>${dline}</div>`;
    ...
    html+=`<div class=title>Tile (${x+1},${y+1})</div>`;
    const st=(t.stock===null?'∞':t.stock);
    html+=`<div class=hint">Type: ${t.type}${(t.type==='forest'||t.type==='mountain'||t.type==='fish')?` · Stock: ${st}`:''}</div>`;
    html+=`<div class=actions><button data-act=close>Close</button></div>`;
  }
  div.innerHTML=html; div.style.left=Math.min(px, window.innerWidth-260)+'px'; div.style.top=Math.min(py, window.innerHeight-200)+'px';
  document.body.appendChild(div);
  div.addEventListener('click',(ev)=>{
    const btn=ev.target.closest('button'); if(!btn) return; const act=btn.dataset.act;
    if(act==='toggle'){
      if(!t.building) return;
      t.building.paused=!t.building.paused;
      const bp= BP[t.building.kind];
      if(bp && bp.cap && t.building.paused && t.building.owner==='player'){
        // Pause: mind. 1 Soldat bleibt, Veteranen räumen
        const total=(t.building.garrisonS|0)+(t.building.garrisonV|0);
        if(total>=1){ t.building.garrisonS = Math.max(1, t.building.garrisonS|0); t.building.garrisonV = 0; }
        else { t.building.garrisonS = 0; t.building.garrisonV = 0; }
      }
...
/* ===================== Geology ===================== */
function sendGeologist(x,y){
  const t=State.map[y][x]; if(t.type!=='mountain' || t.deposit) return;
  t.surveyed=true;
  const r=Math.random(); let dep='iron';
  if(r<0.35) dep='coal'; else if(r>0.85) dep='gold';
  t.deposit=dep; t.dStock = dep==='gold'? 20 + (Math.random()*15|0) : dep==='iron'? 28 + (Math.random()*18|0) : 35 + (Math.random()*20|0);
  renderBoard();
}
...
function canPlace(kind,x,y,owner){
  ...
  const bp=BP[kind]; const R = owner==='player'? State.res : State.ai.res;
  for(const [r,c] of Object.entries(bp.cost||{})){ if((R[r]||0)<c) return {ok:false, reason:'too expensive'}; }
  return {ok:true};
}
function tryBuildAt(kind,x,y,owner='player'){
  ...
  const bp=BP[kind]; const b={kind, timer:bp.every, paused:false, idle:false, idleReason:null, owner}; if(bp.cap){ b.garrisonS=0; b.garrisonV=0; }
...
function demolishAt(x,y){
  const t=State.map[y][x]; if(!t.building) return; const b=t.building; if(b.owner!=='player') return; if(b.kind==='hq') return;
  const bp=BP[b.kind]; const R = b.owner==='player'? State.res : State.ai.res;
  for(const [r,c] of Object.entries(bp.cost||{})){ R[r]=(R[r]||0)+Math.floor(c*0.5); }
  if(bp.cap){ b.garrisonS=0; b.garrisonV=0; }
  t.building=null; updateControl(); renderBoard(); renderResBar(); if(b.owner==='player') autoGarrisonPlayer(); drawMinimap();
}
...
function renderBoard(){
  ...
  for(let vy=0; vy<VIEW_H; vy++){
    for(let vx=0; vx<VIEW_W; vx++){
      const x = State.cam.x + vx, y = State.cam.y + vy; if(!inBounds(x,y)) continue;
      const t=State.map[y][x]; const vis = isVisibleToPlayer(x,y);
      const cell = document.createElement('div'); cell.className = `cell t-${t.type}`; cell.dataset.x=x; cell.dataset.y=y;

      if(vis){
        if(t.building){
          const b=t.building; const bp=BP[b.kind];
          const badge=document.createElement('div'); badge.className='badge'+(b.owner==='ai'?' ai':'');
          const gsum = (bp.cap? ((b.garrisonS||0)+(b.garrisonV||0)) : null);
          if(bp.cap){ badge.textContent = `${bp.icon} ${gsum||0}${b.paused?' ⏸':''}`; } else { badge.textContent = `${bp.icon}${b.paused?' ⏸':''}`; }
          cell.appendChild(badge);

          const label=document.createElement('div');
          const reason = (b.idle && !b.paused)
            ? (b.idleReason==='no-input'?' (no input)':
               b.idleReason==='no-near-resource'?' (no resource)':
               b.idleReason==='no-deposit'?' (no deposit)':
               b.idleReason==='no-rations'?' (no rations)':
               b.idleReason==='no-field'?' (no field)':' (idle)')
            : '';
          label.textContent=(b.owner==='ai'?'👾 ':'')+bp.name+reason;
          label.style.fontSize='10px'; label.style.opacity=.85; cell.appendChild(label);

          if(b.idle && !b.paused){ const xm=document.createElement('div'); xm.className='xmark'; xm.textContent='✖'; cell.appendChild(xm); }
        } else {
          const s=document.createElement('div'); s.textContent=TileEmoji[t.type]; s.style.fontSize='16px'; s.style.opacity=.9; cell.appendChild(s);
          if((t.type==='forest'||t.type==='mountain'||t.type==='fish')){
            const q=document.createElement('div'); q.textContent=(t.stock===null?'∞':t.stock);
            q.style.fontSize='10px'; q.style.opacity=.6; q.style.position='absolute'; q.style.bottom='4px'; q.style.right='6px'; cell.appendChild(q);
          }
        }
        if(t.type==='mountain' && t.deposit){
          const tag=document.createElement('div'); tag.className='tag';
          tag.textContent=(t.deposit==='coal'?'C':t.deposit==='iron'?'Fe':'Au')+':'+t.dStock;
          tag.style.borderColor='#445'; tag.style.background='#0008'; if(t.deposit==='gold') tag.style.color='#f7d308'; cell.appendChild(tag);
        }
      } else {
        const fog=document.createElement('div'); fog.className='fog'; cell.appendChild(fog);
      }
      ...
      cell.addEventListener('click',()=>{ if(!isVisibleToPlayer(x,y)) return;
        if(currentMode==='demolish') demolishAt(x,y);
        else if(currentMode==='build'){
          if(State.map[y][x].building){
            const rect=cell.getBoundingClientRect(); openMenu(rect.left+rect.width/2, rect.top+rect.height/2, x,y);
          } else { tryBuildAt(selectedKind,x,y,'player'); }
        }});
...
function centerCam(){
  let hx=Math.floor(WORLD/2), hy=Math.floor(WORLD/2);
  outer: for(let y=0;y<WORLD;y++) for(let x=0;x<WORLD;x++){
    const b=State.map[y][x].building; if(b && b.kind==='hq' && b.owner==='player'){ hx=x; hy=y; break outer; }
  }
  State.cam.x = Math.max(0, Math.min(WORLD-VIEW_W, hx-Math.floor(VIEW_W/2)));
  State.cam.y = Math.max(0, Math.min(WORLD-VIEW_H, hy-Math.floor(VIEW_H/2)));
  renderBoard(); drawMinimap();
}

/* ===================== Resources & Ops ===================== */
function RES(owner){ return owner==='player'? State.res : State.ai.res; }

// Alte 1-Feld-Ernte (bleibt für Fisch/Hunter Rückfall verfügbar)
function consumeNearby(x,y,need,amt=1){
  const tiles = [{x,y,t:State.map[y][x]}].concat(neighbors(x,y).map(([a,b])=>({x:a,y:b,t:State.map[b][a]}))).filter(n=>n.t.type===need);
  if(tiles.length===0) return false;
  tiles.sort((A,B)=>((B.t.stock??1e9)-(A.t.stock??1e9)));
  const pick=tiles[0];
  if(pick.t.stock===null) return true;
  pick.t.stock -= amt;
  if(pick.t.stock<=0){
    if(need==='fish'){ State.map[pick.y][pick.x].type='water'; State.map[pick.y][pick.x].stock=null; }
    else { pick.t.type='grass'; pick.t.stock=null; }
    renderBoard(); drawMinimap();
  }
  return true;
}

/* === NEU v1.0.3: Ernte im Manhattan-Radius (Holz/Stein) === */
function consumeNearbyR(x, y, type, amt = 1, dist = 2){
  let best=null;
  for(let yy=Math.max(0,y-dist); yy<=Math.min(WORLD-1,y+dist); yy++){
    for(let xx=Math.max(0,x-dist); xx<=Math.min(WORLD-1,x+dist); xx++){
      if(Math.abs(xx-x)+Math.abs(yy-y) > dist) continue;
      const t=State.map[yy][xx]; if(t.type!==type) continue;
      const stock = (t.stock===null)? Infinity : (t.stock|0);
      if(!best || stock>best.stock) best={xx,yy,t,stock};
    }
  }
  if(!best) return false;
  if(best.t.stock===null) return true;
  best.t.stock -= amt;
  if(best.t.stock<=0){
    if(type==='forest'){ best.t.type='grass'; best.t.stock=null; }
    else if(type==='fish'){ best.t.type='water'; best.t.stock=null; }
    else if(type==='mountain'){ best.t.type='grass'; best.t.stock=null; }
  }
  renderBoard(); drawMinimap();
  return true;
}

function hasDepositNearby(x,y,dep){
  const tiles = [{x,y,t:State.map[y][x]}].concat(neighbors(x,y).map(([a,b])=>({x:a,y:b,t:State.map[b][a]})));
  return tiles.some(n=> n.t.type==='mountain' && n.t.deposit===dep && n.t.dStock>0);
}
function consumeDepositNearby(x,y,dep,amt=1){
  const tiles = [{x,y,t:State.map[y][x]}].concat(neighbors(x,y).map(([a,b])=>({x:a,y:b,t:State.map[b][a]}))).filter(n=> n.t.type==='mountain' && n.t.deposit===dep && n.t.dStock>0);
  if(tiles.length===0) return false;
  tiles.sort((A,B)=> (B.t.dStock - A.t.dStock));
  const pick=tiles[0]; pick.t.dStock -= amt;
  if(pick.t.dStock<=0){ pick.t.deposit=null; pick.t.dStock=0; }
  return true;
}
function consumeRation(owner){
  const pref=['fish','meat','bread']; const R=RES(owner);
  for(const r of pref){ if((R[r]||0)>=1){ R[r]--; return true; } }
  return false;
}

/* ===================== Auto Garrison (inkrementell) ===================== */
function autoGarrisonPlayer(){
  // Forts sammeln
  const forts=[];
  for(let y=0;y<WORLD;y++) for(let x=0;x<WORLD;x++){
    const b=State.map[y][x].building;
    if(b && b.owner==='player' && BP[b.kind].cap && !b.paused){
      const frontier = neighbors(x,y).some(([ax,ay])=> !State.control.player[ay][ax]);
      const prio = (frontier?1000:0) + (b.kind==='castle'?100:0) + (((x+y)|0)%7);
      forts.push({b,x,y,frontier,prio});
    }
  }
  forts.sort((A,B)=>B.prio-A.prio);
  const castles = forts.filter(f=>f.b.kind==='castle').map(f=>f.b);
  const towers  = forts.filter(f=>f.b.kind==='watch').map(f=>f.b);

  // freie Kontingente (ohne Reset!)
  const lockedInTowers = towers.reduce((n,b)=> n + (((b.garrisonS|0)+(b.garrisonV|0)) ? 1 : 0), 0);
  let freeS = Math.max(0, (State.soldiers|0) - (State.veterans|0) - lockedInTowers);
  let freeV = Math.max(0,  State.veterans|0);

  // 1) leere Türme → 1 Soldat
  for(const b of towers){
    if(((b.garrisonS|0)+(b.garrisonV|0))===0 && freeS>0){ b.garrisonS+=1; freeS--; }
  }
  // 2) Burgen bevorzugt mit Veteranen füllen
  for(const b of castles){
    const cap=BP[b.kind].cap;
    if((b.garrisonS|0)+(b.garrisonV|0)===0 && freeV>0){ b.garrisonV+=1; freeV--; }
    let free = cap - ((b.garrisonS|0)+(b.garrisonV|0));
    if(free>0 && freeV>0){ const putV=Math.min(free,freeV); b.garrisonV+=putV; freeV-=putV; free-=putV; }
    if(free>0 && freeS>0){ const putS=Math.min(free,freeS); b.garrisonS+=putS; freeS-=putS; }
  }
  // 3) Restliche Soldaten auf Türme
  for(const b of towers){
    const cap=BP[b.kind].cap; const free = cap - ((b.garrisonS|0)+(b.garrisonV|0));
    if(free>0 && freeS>0){ const put=Math.min(free,freeS); b.garrisonS+=put; freeS-=put; }
  }
  // 4) Sicherheit: leere Türme ziehen 1 Soldaten von Spendern (>1)
  const empties = towers.filter(b => ((b.garrisonS|0)+(b.garrisonV|0))===0);
  if(empties.length){
    const donors = forts.map(f=>f.b)
      .filter(b => (b.owner==='player' && BP[b.kind].cap && (b.garrisonS|0) > 1))
      .sort((A,B)=> ( (A.kind==='castle'?0:1) - (B.kind==='castle'?0:1) ) ||
                    ( ((B.garrisonS|0)+(B.garrisonV|0)) - ((A.garrisonS|0)+(A.garrisonV|0)) ));
    for(const t of empties){
      for(const d of donors){
        if((d.garrisonS|0) > 1){ d.garrisonS--; t.garrisonS++; break; }
      }
    }
  }
...
function autoGarrisonAI(){
  for(let y=0;y<WORLD;y++) for(let x=0;x<WORLD;x++){
    const b=State.map[y][x].building;
    if(b && b.owner==='ai' && BP[b.kind].cap){ b.garrisonS=0; b.garrisonV=0; }
  }
  let availS = State.ai.soldiers|0;
  const forts=[]; for(let y=0;y<WORLD;y++) for(let x=0;x<WORLD;x++){ const b=State.map[y][x].building; if(b && b.owner==='ai' && BP[b.kind].cap && !b.paused){ forts.push(b); } }
...
/* ===================== Capture & Endgame ===================== */
function destroyAround(ownerVictim,cx,cy,r){
  let destroyed=0, lostUnits=0; const r2=r*r;
  for(let y=Math.max(0,cy-r); y<Math.min(WORLD,cy+r+1); y++)
    for(let x=Math.max(0,cx-r); x<Math.min(WORLD,cx+r+1); x++){
      const dx=x-cx, dy=y-cy; if(dx*dx+dy*dy>r2) continue;
      const t=State.map[y][x]; const b=t.building;
      if(b && b.owner===ownerVictim){
        if(BP[b.kind].cap){ lostUnits += (b.garrisonS||0)+(b.garrisonV||0); }
        t.building=null; destroyed++;
      }
    }
  return {destroyed,lostUnits};
}
function captureAt(x,y,newOwner,captor){
  const t=State.map[y][x]; const b=t.building; if(!b) return;
  const oldOwner=b.owner; if(oldOwner===newOwner) return;
  const bp=BP[b.kind]; const gLost = (b.garrisonS||0)+(b.garrisonV||0);
  if(newOwner==='player'){ State.stats.aiUnitsLost += gLost; State.stats.playerUnitsUsed += Math.max(1, sumGarrison(captor)); State.stats.capturesByPlayer++; }
  else { State.stats.playerUnitsLost += gLost; State.stats.aiUnitsUsed += Math.max(1, sumGarrison(captor)); State.stats.capturesByAI++; }
  b.owner=newOwner; b.garrisonS=0; b.garrisonV=0;
  if(b.kind==='hq'){ endGame(newOwner==='player' ? 'win' : 'lose'); return; }
...
function checkCaptures(){
  for(let y=0;y<WORLD;y++) for(let x=0;x<WORLD;x++){
    const t=State.map[y][x]; const b=t.building; if(!b) continue;
    if(b.owner==='ai' && (b.kind==='watch'||b.kind==='castle'||b.kind==='hq')){
      const cap = friendlyCoverageAt('player',x,y); if(cap){ captureAt(x,y,'player',cap); }
    }
  }
  for(let y=0;y<WORLD;y++) for(let x=0;x<WORLD;x++){
    const t=State.map[y][x]; const b=t.building; if(!b) continue;
    if(b.owner==='player' && (b.kind==='watch'||b.kind==='castle'||b.kind==='hq')){
      const cap = friendlyCoverageAt('ai',x,y); if(cap){ captureAt(x,y,'ai',cap); }
    }
  }
}
function endGame(result){
  if(State.ended) return; State.ended=true; elEnd.classList.add('show');
  elEndTitle.textContent = result==='win' ? 'Victory!' : 'Defeat!';
  const s=State.stats;
...
function renderQuests(){
  const items = State.quests.map(q=>{
    const ok=q.check(); const rew = q.reward? ` — Reward: ${rewardStr(q.reward)}${q.awarded?' (claimed)':''}` : '';
    return `<div>• ${ok?'✅':'⬜'} ${q.text}${rew}</div>`;
  }).join('');
  elQuests.innerHTML = items;
}
function processQuestRewards(){
  let changed=false;
  for(const q of State.quests){
    if(!q.awarded && q.check()){
      if(q.reward){ for(const [k,v] of Object.entries(q.reward)){ State.res[k]=(State.res[k]||0)+v; } }
      q.awarded=true; changed=true;
    }
  }
  if(changed){ renderResBar(); renderQuests(); }
}
...
function tick(ms){
  ...
  for(let y=0;y<WORLD;y++) for(let x=0;x<WORLD;x++){
    const t=State.map[y][x]; const b=t.building; if(!b) continue;
    const bp=BP[b.kind]; if(!bp.every) continue;

    if(b.paused){ b.timer = bp.every; b.idle=false; b.idleReason=null; continue; }
    ...
    // Inputs
    if(bp.in){
      let ok=true; for(const [r,c] of Object.entries(bp.in)){ if((R[r]||0) < c){ ok=false; break; } }
      if(!ok){ b.idle=true; b.idleReason='no-input'; b.timer += Math.max(250,bp.every); continue; }
      for(const [r,c] of Object.entries(bp.in)) R[r]-=c;
    }

    // Forester / Fish Farm
    if(b.kind==='forester'){
      const cands = neighbors(x,y).filter(([a,b2])=> State.map[b2][a].type==='grass' && !State.map[b2][a].building);
      if(cands.length){ const [ax,ay]=cands[(Math.random()*cands.length)|0]; State.map[ay][ax].type='forest'; State.map[ay][ax].stock=30; renderBoard(); drawMinimap(); }
      b.timer += Math.max(250,bp.every); continue;
    }
    if(b.kind==='fish_farm'){
      const cands = neighbors(x,y).filter(([a,b2])=> State.map[b2][a].type==='water' && !State.map[b2][a].building);
      if(cands.length){ const [ax,ay]=cands[(Math.random()*cands.length)|0]; State.map[ay][ax].type='fish'; State.map[ay][ax].stock=25; renderBoard(); drawMinimap(); }
      b.timer += Math.max(250,bp.every); continue;
    }

    // Gatherer (NEU: lumber/mason Reichweite 2)
    if(['lumber','mason','fisher','hunter'].includes(b.kind)){
      const need = BP[b.kind].needs;
      const reach = (b.kind==='lumber' || b.kind==='mason') ? 2 : 1;
      const ok = reach===1 ? consumeNearby(x,y,need,1) : consumeNearbyR(x,y,need,1,reach);
      if(need && !ok){ b.idle=true; b.idleReason='no-near-resource'; b.timer += Math.max(250,bp.every); continue; }
    }
...
function aiStep(ms){
  const A = State.ai; A.actionTimer += ms; A.soldierTimer += ms;
  if(A.soldierTimer > 30000){ A.soldierTimer = 0; if(A.soldiers < 6){ A.soldiers++; autoGarrisonAI(); } }
  if(A.actionTimer < 2600) return; A.actionTimer = 0;
  const plan = ['lumber','sawmill','mason','farm','acre','acre','well','mill','bakery','watch','watch','watch','fisher','fish_farm'];
  if(A.planStep < plan.length){ const k = plan[A.planStep]; if(aiTryBuild(k)) A.planStep++; return; }
  if((A.res.stone||0) >= 24){ aiTryBuild('watch'); }
}
function aiFindSpot(kind){
  const tries = 400;
  for(let i=0;i<tries;i++){
    const x=(Math.random()*WORLD)|0, y=(Math.random()*WORLD)|0;
    if(!State.control.ai[y][x]) continue;
    const c=canPlace(kind,x,y,'ai'); if(!c.ok) continue;
    if(kind==='watch'){ const nearEdge = neighbors(x,y).some(([a,b])=> !State.control.ai[b][a]); if(!nearEdge) continue; }
    return {x,y};
  }
  return null;
}
function aiTryBuild(kind){
  const bp=BP[kind]; const R=State.ai.res;
  for(const [r,c] of Object.entries(bp.cost||{})){ if((R[r]||0)<c) return false; }
  const spot = aiFindSpot(kind); if(!spot) return false;
  tryBuildAt(kind, spot.x, spot.y, 'ai'); return true;
}
...
function boot(){
  ...
  State.map=newMap(); State.res={}; State.ai.res={}; for(const r of ResList){ State.res[r]=0; State.ai.res[r]=0; }
  ...
  State.map[cy][cx].building={kind:'hq', owner:'player', timer:0, paused:false, idle:false};
  const aiCorners = [[4,4],[WORLD-5,4],[4,WORLD-5],[WORLD-5,WORLD-5]];
  const pick = aiCorners[(rand()*aiCorners.length)|0]; const ax=pick[0], ay=pick[1];
  State.map[ay][ax].building={kind:'hq', owner:'ai', timer:0, paused:false, idle:false};

  for(const [sx,sy] of [[cx,cy],[ax,ay]]){
    for(let dy=-2;dy<=2;dy++) for(let dx=-2;dx<=2;dx++){
      const X=sx+dx, Y=sy+dy; if(inBounds(X,Y)){ State.map[Y][X].type='grass'; State.map[Y][X].stock=null; }
    }
  }

  updateControl(); centerCam(); renderResBar(); renderPalette(); setupQuests(); renderBoard(); drawMinimap();
  _last=performance.now(); _accum=0; _loopId=requestAnimationFrame(loop);
}
window.boot = boot;

/* ===================== Game facade ===================== */
const Game = {
  start(){ try{ audio.play().catch(()=>{}); }catch(e){} const overlay = document.getElementById('start'); if(overlay) overlay.classList.remove('show'); State.started = true; },
